 ...
 * Se activa cada ALERT_PERIOD_MS ms y también mediante semáforo cuando
 * la tarea BME680 produce nuevos datos.
 *
 * Lógica de color del LED:
 *   Verde sólido   — todo nominal
 *   Amarillo (R+G) — alerta de calidad de aire o humedad alta
 *   Rojo sólido    — temperatura alta o baja
...
/* ── Nodos del devicetree para el LED RGB ────────────────────────────────── */
...
/* ── Inicialización de GPIOs del LED ─────────────────────────────────────── */
...
/* ── Selección del estado del LED basado en alertas ──────────────────────── */
static led_state_t select_led_state(const alert_status_t *alr)
{
    if (!alr->any_active)                  return LED_GREEN;
    if (alr->motion_detected && !alr->temp_high && !alr->temp_low)
...
void task_alert_entry(void *p1, void *p2, void *p3)
{
    ...
    if (led_init() != 0) {
        LOG_ERR("Fallo al inicializar LED RGB. Tarea terminada.");
        return;
    }
    ...
    while (1) {
        /* Esperar nuevo dato o timeout de 500 ms */
        ...
        /* Actualizar estado global de alertas */
        k_mutex_lock(&g_data_mutex, K_FOREVER);
        g_alerts = alr;
        k_mutex_unlock(&g_data_mutex);

        /* Actualizar LED */
        led_state_t led_st = select_led_state(&alr);
        blink_phase = !blink_phase;
        led_set(led_st, blink_phase);
